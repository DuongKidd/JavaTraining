...
public class BankController {
    ...
    public ResponseEntity<?> deleteBank(@PathVariable int id) {
        try {
            ...
            return ResponseEntity.ok(new APIResponse<>(HttpStatus.OK.value(), "Delete bank successfully!"));
        } catch (Exception e) {
            log.info(e.getMessage(), e);
            return ResponseEntity.ok(new APIResponse<>(HttpStatus.INTERNAL_SERVER_ERROR.value(), e.getMessage()));
        }
