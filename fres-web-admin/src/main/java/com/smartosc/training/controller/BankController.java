...
public class BankController {
    ...
    public String showEditBankPage(@RequestParam(value = "bankId") Integer bankId, Model model) {
        ...
        if (bankId == null) {
            ...
            BankDTO bank = restService.execute(
                    ...
                    header,
                    null,
                    new ParameterizedTypeReference<APIResponse<BankDTO>>() {}).getData();
    ...
    public String updateBank(@Valid @ModelAttribute("banks") BankDTO bankDTO,
        ...
        Boolean status = restService.execute(
                ...
                header,
                bankDTO,
                new ParameterizedTypeReference<APIResponse<Boolean>>() {}).getData();
    ...
    public ResponseEntity<?> deleteBank(@PathVariable Integer bankId) {
        ...
        header.setContentType(MediaType.APPLICATION_JSON);
        try{
            APIResponse apiResponse = restService.execute(
                    url + "banks/" + bankId,
                    HttpMethod.DELETE,
                    header,
                    null,
